...
const searchId = inventory.find(element => element.id == 33)

console.log(`Car 33 is a ${searchId.car_year} ${searchId.car_make} ${searchId.car_model}`);
...
const fNamesCaps = runners.map(item => firstNamesAllCaps.push(item.first_name.toUpperCase())   )
...
const largeSizeShirt = runners.filter(item => {
    if (item.shirt_size === 'XL' || item.shirt_size === '2XL' || item.shirt_size === '3XL') {
...
const reduceDonation = runners.reduce((a ,b) => a + b, ticketPriceTotal )

console.log(`t ${ticketPriceTotal}`);
